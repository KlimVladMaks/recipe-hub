...
export const isCommentAuthor = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const userId = req.currentUserId!;
    const commentId = parseInt(req.params.commentId);
    const { CommentService } = await import('../services/comment.service.js');
    const isAuthor = await CommentService.isUserCommentAuthor(userId, commentId);
    if (!isAuthor) {
...
export const isCommentAuthorOrAdmin = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const userId = req.currentUserId!;
    const commentId = parseInt(req.params.commentId);
    const { CommentService } = await import('../services/comment.service.js');
