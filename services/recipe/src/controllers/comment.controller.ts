...
export class CommentController {
    ...
    static async isCommentAuthor(req: AuthRequest, res: Response, next: any) {
        try {
            const currentUserId = req.currentUserId!;
            const { commentId:commentIdStr } = req.params;
            const commentId = parseInt(commentIdStr as string);
            ...
            if (!isAuthor) {
                res.status(403).json({ message: "Доступ только для автора комментария" });
                return;
            }
            next();
        } catch (error: any) {
    ...
    static async isCommentAuthorOrAdmin(req: AuthRequest, res: Response, next: any) {
        try {
            const currentUserId = req.currentUserId!;
            const { commentId:commentIdStr } = req.params;
            const commentId = parseInt(commentIdStr as string);
            ...
            if (!(isAuthor || userIsAdmin)) {
                res.status(403).json({ message: "Доступ только для автора комментария и администраторов" });
                return;
            }
            next();
        } catch (error: any) {
    ...
    static async isCorrectCommentId(req: AuthRequest, res: Response, next: any) {
        try {
            ...
            if (!isCorrect) {
                res.status(400).json({ message: "Комментарий не относится к указанному рецепту" });
                return;
            }
            next();
        } catch (error: any) {
